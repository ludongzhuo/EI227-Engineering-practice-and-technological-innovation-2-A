...
uint8_t systick_1s_status=0,systick_200ms_status=0;
uint32_t systick_1s_counter=1000,systick_200ms_counter=200;
uint32_t min=59,sec=45;
uint8_t num=0;
  ...
  ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_16MHZ |SYSCTL_OSC_INT |SYSCTL_USE_OSC), 16000000);		
	...
	while (1)
	{
		...
		switch(num)
		{
			...
			case 1:result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[min%10]);
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<num);break;	
		  case 2:result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,0x40);
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<num);break;	
			case 3:result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[sec/10]);
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<num);break;	
			default:result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[sec%10]);
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<num);break;		
		}
		...
		if(systick_1s_status==1)
		{
			systick_1s_status=0;
			if(!pressed1&&!pressed2) sec++;
		}
		...
		if(GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0)==0) pressed1=true;
	  else{
			if(pressed1)
			{
				pressed1=false;
		...
		if(GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_1)==0) pressed2=true;
	  else{
			if(pressed2)
			{
				pressed2=false;
